...
if ($role_code === 'CEO' || $role_code === 'RISK_ADMIN') {
    ...
    $risk_query = "SELECT r.*, 
                          CONCAT(ro.first_name, ' ', ro.last_name) as risk_owner_name,
                          CONCAT(su.first_name, ' ', su.last_name) as system_user_name,
                          d.department_name,
                          rc.category_name
                   FROM risks r
                   LEFT JOIN users ro ON r.risk_owner_id = ro.user_id
                   LEFT JOIN users su ON r.system_user_id = su.user_id
                   LEFT JOIN departments d ON r.department_id = d.department_id
                   LEFT JOIN risk_categories rc ON r.risk_category_id = rc.category_id
                   WHERE r.risk_owner_id = ? OR r.system_user_id = ?
                   ORDER BY r.priority DESC, r.next_review_date ASC LIMIT 10";
    $risk_stmt = $conn->prepare($risk_query);
    $risk_stmt->execute([$user_id, $user_id]);
}
